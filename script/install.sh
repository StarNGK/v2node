...
install_v2node() {
    ...
    if  [[ -z "$version_param" ]] ; then
        last_version=$(curl -Ls "https://api.github.com/repos/starngk/v2node/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        url="https://github.com/starngk/v2node/releases/download/${last_version}/v2node-linux-${arch}.zip"
...
EOF
    ...
    curl -o /usr/bin/v2node -Ls https://raw.githubusercontent.com/starngk/v2node/main/script/v2node.sh
    chmod +x /usr/bin/v2node

    cd $cur_dir
    rm -f install.sh
    echo "------------------------------------------"
    echo -e "${green}${plain}管理脚本使用方法: "
    echo "------------------------------------------"
